...
menu.innerHTML = grupos.map(([rol, enlaces]) => `<div class="menu_grupo menu_grupo_${rol}"><span class="menu_grupo_titulo">${rol[0].toUpperCase() + rol.slice(1)}</span>${enlaces.map(([href, texto]) => `<a href="${href}" class="menu_item">${texto}</a>`).join('')}</div>`).join('') + '<a href="../backend/api/logout.php" class="menu_item menu_cerrar" id="cerrar_sesion">Cerrar sesión</a>';
